		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					func(s *terraform.State) error {
						if !strings.HasSuffix(*conf.Configuration.DeadLetterConfig.TargetArn, ":"+funcName) {
							return fmt.Errorf(
								"Expected DeadLetterConfig.TargetArn %s to have suffix %s", *conf.Configuration.DeadLetterConfig.TargetArn, ":"+funcName,
							)
						}
						return nil
					},
...
func TestAccAWSLambdaFunction_DeadLetterConfigUpdated(t *testing.T) {
	...
	topic2Name := fmt.Sprintf("tf_acc_topic_lambda_func_dlcfg_upd_%s", rString)
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAwsLambdaFunctionExists("aws_lambda_function.lambda_function_test", funcName, &conf),
					func(s *terraform.State) error {
						if !strings.HasSuffix(*conf.Configuration.DeadLetterConfig.TargetArn, ":"+uFuncName) {
							return fmt.Errorf(
								"Expected DeadLetterConfig.TargetArn %s to have suffix %s", *conf.Configuration.DeadLetterConfig.TargetArn, ":"+uFuncName,
							)
						}
						return nil
					},
...
func TestAccAWSLambdaFunction_s3(t *testing.T) {
	...
	rString := acctest.RandString(8)
	bucketName := fmt.Sprintf("tf_acc_bucket_lambda_func_s3_%s", rString)
...
func TestAccAWSLambdaFunction_s3Update_basic(t *testing.T) {
	...
	rString := acctest.RandString(8)
	bucketName := fmt.Sprintf("tf_acc_bucket_lambda_func_s3_upd_basic_%s", rString)
...
func TestAccAWSLambdaFunction_s3Update_unversioned(t *testing.T) {
	...
	rString := acctest.RandString(8)
	bucketName := fmt.Sprintf("tf_acc_bucket_lambda_func_s3_upd_unver_%s", rString)
